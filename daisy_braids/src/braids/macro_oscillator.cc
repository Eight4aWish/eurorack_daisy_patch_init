...
void MacroOscillator::RenderMorph(const uint8_t* sync, int16_t* buffer, size_t size) {
  ...
  const int16_t parameter_1 = parameter_[0];
  ...
  // Smooth the mix to avoid zippering.
...
void MacroOscillator::RenderSineTriangle(const uint8_t* sync, int16_t* buffer, size_t size) {
  ...
  attenuation_tri = std::clamp<int32_t>(attenuation_tri, 0, 32767);
  attenuation_sine = std::clamp<int32_t>(attenuation_sine, 0, 32767);
...
void MacroOscillator::RenderTriple(const uint8_t* sync, int16_t* buffer, size_t size) {
  ...
  while (size--) {
    ...
    *buffer = static_cast<int16_t>(std::clamp<int32_t>(s / 3, -32768, 32767));
    buffer++;
    temp_buffer++;
  }
}
