...
- `daisy_neural/` – **in progress**: neural audio models on the patch.init — a NAM A2 capture of the Befaco Chopping Kinky first, then a CV-conditioned GRU of the Dual Pingable LPG from [eurorack_electronics](https://github.com/Eight4aWish/eurorack_electronics), where the vactrol's memory is something a static curve cannot reproduce. The phase 1 skeleton is built (trim, level, bypass, peak meter, CPU load) with the model slot running as a passthrough, which doubles as the pass-through firmware the AC/DC coupling bench check needs — see its [README](daisy_neural/README.md) and [CLAUDE.md](daisy_neural/CLAUDE.md)
